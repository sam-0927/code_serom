"""
Stage-1 training: WavLMLoRASE (mel_head 포함, Transformer decoder 없음)
  - Main loss  : mel_l1  (L1 on log-mel from se_model.mel_head vs clean reference)
  - Aux losses : CTC
  - No vocoder, no discriminator, no transformer decoder

Checkpoint format:
  { epoch, se_cfg, se_model, optimizer_se, scheduler_se }
"""
...
class Stage1Trainer:
    def __init__(self, config, se_model,
                 optimizer_se, scheduler_se,
    ...
    def _forward(self, noisy_wav, clean_wav, audio_lengths,
        ...
        # Build input feature and directly predict mel via se_model.mel_head
        ...
        frame_mask = (
            torch.arange(T_min, device=input_feat.device).unsqueeze(0)
    ...
    def _train_epoch(self, epoch):
        ...
        self.writer.add_scalar("lr/se", self.optimizer_se.param_groups[0]["lr"], epoch)
    ...
    def train(self):
        ...
        print(f"Stage-1 (no-transformer) training for {self.epochs} epochs done.")
...
def build_se_model(config, args, device):
    ...
    # n_mels from training config → injected into se_cfg so WavLMLoRASE builds the right mel_head
...
def run(config, args):
    ...
    se_model, se_cfg = build_se_model(config, args, device)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Stage-1 (no transformer): WavLMLoRASE with built-in mel_head"
    )
