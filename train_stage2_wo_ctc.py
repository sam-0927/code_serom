"""
Stage-2 training (no CTC): Vocoder only (se_model + decoder frozen from stage-1 wo_ctc)
  - Loads stage-1 wo_ctc checkpoint and freezes se_model + decoder
  - Trains generator (WavLMDec) + discriminator (MPD + MBD)
  - Losses: MultiScaleMelSpectrogramLoss (recons), adversarial, feature matching
  - context emb: WavLM content hidden → proj_content (FC) combined with acoustic_proj
...
class Stage2Trainer:
    def __init__(self, config, se_model, decoder, generator, discriminator,
        ...
        with open(os.path.join(self.exp_path, "config_stage2_wo_ctc.yaml"), "w") as f:
    ...
    def _frozen_forward(self, noisy_wav, audio_lengths):
        ...
        # No CTC: project WavLM content output directly via proj_content (FC)
    ...
    def train(self):
        ...
        print(f"Stage-2 (no CTC) training for {self.epochs} epochs done.")
...
def build_models(config, args, device):
    # ── Load stage-1 wo_ctc checkpoint to restore se_model + decoder ──────────
    stage1_ckpt_path = config.get("stage1_ckpt_path", "")
    if not stage1_ckpt_path:
        raise ValueError("config['stage1_ckpt_path'] must point to a stage-1 wo_ctc checkpoint.")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Stage-2 (no CTC): vocoder fine-tuning with frozen stage-1 wo_ctc front-end"
    )
