"""
Stage-1 training (no CTC): WavLMLoRASE + TransformerDecoder
  - Main loss  : mel_l1  (L1 on log-mel from decoder.mel_head vs clean reference)
  - WavLM content hidden → se_model.proj_content (FC) combined with acoustic_proj → decoder
  - No CTC loss, no vocoder, no discriminator

Checkpoint format:
  { epoch, se_cfg, se_model, decoder,
    optimizer_se, optimizer_dec, scheduler_se, scheduler_dec }
"""
...
class Stage1Trainer:
    def __init__(self, config, se_model, decoder,
        ...
        with open(os.path.join(self.exp_path, "config_stage1_wo_ctc.yaml"), "w") as f:
    ...
    def _forward(self, noisy_wav, clean_wav, audio_lengths):
        ...
        # No CTC: project WavLM content output directly via se_model.proj_content
        acoustic_h = self.se_model.proj_acoustic(acoustic_emb)
        content_h  = self.se_model.proj_content(content_emb)
    ...
    def _validation_epoch(self, epoch):
        ...
        for step, (noisy_wav, clean_wav, audio_lengths, _text_ids, _text_lengths, _infos) in enumerate(bar, 1):
            ...
            mel_l1_loss, mel_pred_vis, target_mel_vis, frame_lens_vis = (
        ...
        val_mel_l1 = total_mel_l1 / step
        self.writer.add_scalars("val_loss", {"mel_l1": val_mel_l1}, epoch)
        return lam_mel_l1 * val_mel_l1, val_mel_l1
    ...
    def train(self):
        ...
        print(f"Stage-1 (no CTC) training for {self.epochs} epochs done.")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Stage-1 (no CTC): WavLMLoRASE + TransformerDecoder (mel_l1 only)"
    )
