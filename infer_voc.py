"""
Inference script for Stage-2 trained model
(WavLM + LoRA + TransformerDecoder + WavLMDec vocoder).
...
# ─────────────────────────────────────────────────────────────────────────────
# Model builder  (mirrors build_models in train_voc.py)
# ─────────────────────────────────────────────────────────────────────────────

def build_models(ckpt_path: str, config: dict, device: torch.device, se_cfg: dict = None):
    """
    ckpt_path : path to .tar saved by Trainer._save_checkpoint
    config    : main training config (config_voc.yaml)
...
# ─────────────────────────────────────────────────────────────────────────────
# Encode helpers  (mirrors Trainer._encode in train_voc.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def run_sample(se_model, decoder, generator, noisy_path: str,
    ...
    # ── 2. Build decoder input (mirrors _frozen_forward in train_stage2.py) ───
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Inference: noisy → enhanced speech, saved by reverb/SNR"
    )
    ...
    p.add_argument("--output_dir", default="enhanced",
...
def main():
    ...
    for clean_path, noisy_path, reverb, snr in tqdm(samples, desc="inference", dynamic_ncols=True):
        ...
        #noisy_wav = librosa.load(noisy_path, sr=fs, mono=True)[0]
        #sf.write(str(out_dir / f"{stem}_input.wav"), noisy_wav, fs)
