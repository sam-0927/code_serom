"""
Stage-2 training: Vocoder only (se_model frozen from stage-1 no-trans)
  - Loads stage-1 (no-trans) checkpoint and freezes se_model (mel_head 포함)
  - Trains generator (WavLMDec) + discriminator (MPD + MBD)
  - Losses: MultiScaleMelSpectrogramLoss (recons), adversarial, feature matching

Checkpoint format:
  { epoch, se_cfg, se_model,
...
class Stage2Trainer:
    def __init__(self, config, se_model, generator, discriminator,
    ...
    def _set_train_mode(self):
        # se_model stays in eval mode (frozen)
        self.se_model.eval()
    ...
    def _frozen_forward(self, noisy_wav, audio_lengths):
        """Run frozen se_model (with mel_head); returns predicted mel and frame lengths."""
        ...
        mel_pred = self.se_model.mel_head(input_feat)   # (B, T, n_mels)
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + self.start_epoch):
            ...
            print(f"[epoch {epoch}] val_total={val_loss:.5f}  val_mel={val_mel:.5f}")
        ...
        print(f"Stage-2 (no-transformer) training for {self.epochs} epochs done.")
...
def build_models(config, args, device):
    # ── Load stage-1 (no-trans) checkpoint to restore se_model (mel_head 포함) ─
    stage1_ckpt_path = config.get("stage1_ckpt_path", "")
    if not stage1_ckpt_path:
        raise ValueError("config['stage1_ckpt_path'] must point to a stage-1 (no-trans) checkpoint.")

    print(f"[stage1] Loading frozen se_model from: {stage1_ckpt_path}")
    ...
    # n_mels from training config → ensures mel_head dim matches
    ...
    se_model.eval()
    print("[stage1] se_model (+ mel_head) frozen.")
    ...
    return se_model, generator, discriminator, se_cfg
...
def run(config, args):
    ...
    se_model, generator, discriminator, se_cfg = build_models(config, args, device)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Stage-2 (no transformer): vocoder fine-tuning with frozen stage-1 no-trans front-end"
    )
